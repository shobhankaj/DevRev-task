...
function SearchedFlight() {
  ...
  const fetchInfo = () => {
    return axios.post("http://localhost:8500/api/user/flight-searched", {from,to,day}).then((res) => setData(res.data));
  };
  ...
  return (
    <div>
        ...
        { data.map((dataObj, index) => {
          return(
            <SearchedItem flightname={dataObj.flightname} from={dataObj.from} to={dataObj.to} id={dataObj.id} user={user}/>
          );
        })}
